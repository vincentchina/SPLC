...
CCoilBlock::BlockSize CCoilBlock::GetMaxSize()
{ 
	return m_szBlock; 
}
CCoilBlock::BlockSize CCoilBlock::SetMaxSize( LPBlockSize pSize )
{ 
...
CCoilBlock::LPCoilLine CCoilBlock::GetLine( int nLn )
{
...
CCoilBlock::LPCoilLine CCoilBlock::UpdateLine( int nLn )
{
...
CCoilBlock::LPCoilPoint CCoilBlock::GetCoilPoint( CoilPos& pos )
{
	LPCoilLine pLine = GetLine( pos.nLn );
	if( pos.nCol < 0 || pos.nCol > m_szBlock.nCol )
...
CCoilBlock::CoilPos CCoilBlock::SetPos( CoilPos& pos )
{
...
BOOL CCoilBlock::AndBlock()
{
	...
	for ( iLine.pos.nLn = iLine.nStart; iLine.pos.nLn <= iLine.nMaxRow; iLine.pos.nLn++,m_ILine.pos.nLn++ )
	{
		...
		while( pAndLine[ iLine.pos.nCol ].hCoil != NULL || pAndLine[ iLine.pos.nCol ].vCoil != NULL )
		{
...
BOOL CCoilBlock::OrBlock()
{
	...
	for ( posLine.nLn = m_ILine.pos.nLn; posLine.nLn < iLine.pos.nLn; posLine.nLn++ )
	{
...
BOOL CCoilBlock::InputLine2Ins( int nLn, CString& strIns )
{
	CoilPos pos;
	BOOL bRes;
	if (nLn < 0 )
...
BOOL CCoilBlock::MinBlock2Ins( CoilPos pos,CString& strOut )
{
